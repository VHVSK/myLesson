...
// 1
// UserContext - функція повертає об'ект, який містить контекст
// ми хможемо змінювати цей контект та читати данні цього контексту

// 2 - використання об'єкта, контекст у вигляді об'єкта
// Змінимо контекст таким чином, щоб в контексті в нас зберагався об'єкт.
// Створили об'єкт, додали юзера та сетЮзера
// СетЮзер в нас значення функція, setUser: () => {}, це для того, якщо десь відсутній провайдер, а ми пробуємо визвати setUser, то щоб в нас не з'явилася помилка. Такі операція являються признаком гарного тону при створені контекста.
