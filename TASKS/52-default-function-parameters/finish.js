...
const weatherForecast = (city, weather) => {
    ...
    // варіант автора, але його відкинули, так як якщо пуста строка передається, то не працює так як потрібно, ну типу пуста строка по завданню, це типу також значення в нашому випадку.
    // weather = weather || 'Отличная погода!'

    // тому автор зупинився на такому варіанті
    // weather = weather === undefined ? 'Отличная погода!' : weather
    // weather = weather !== undefined ? weather : 'Отличная погода!'

    return `Прогноз погоды для города ${city}: ${weather}!`
}

// Остаточний варіант атвора
/**
